...
export default class Synapse extends Base {
  ...
  async getTriggers(workspace: string): Promise<Trigger[]> {
    return (
      await this.get(`https://${workspace}.dev.azuresynapse.net/triggers`, {
  ...
  async getDataflows(workspace: string): Promise<DataFlow[]> {
    return (
      await this.get(`https://${workspace}.dev.azuresynapse.net/dataflows`, {
        'api-version': '2020-12-01',
      })
    ).value
  }
  ...
  async getSqlScripts(workspace: string): Promise<SQLScript[]> {
    return (
      await this.get(`https://${workspace}.dev.azuresynapse.net/sqlScripts`, {
        'api-version': '2020-12-01',
      })
    ).value
  }
  ...
  async getSqlPools(workspace: string): Promise<SQLPool[]> {
    return (
      await this.get(`https://${workspace}.dev.azuresynapse.net/sqlPools`, {
        'api-version': '2020-12-01',
      })
    ).value
  }
  ...
  async getPipelines(workspace: string): Promise<Pipeline[]> {
    return (
      await this.get(`https://${workspace}.dev.azuresynapse.net/pipelines`, {
        'api-version': '2020-12-01',
      })
    ).value
  }
  ...
  async getDatasets(workspace: string): Promise<Dataset[]> {
    return (
      await this.get(`https://${workspace}.dev.azuresynapse.net/datasets`, {
        'api-version': '2020-12-01',
      })
    ).value
  }
